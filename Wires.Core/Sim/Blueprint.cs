...
public class Blueprint
{
    ...
    public void Draw(Graphics g, Simulation? sim, Point pos, float scale, float wireRad, bool isShortCircuit, int inputOutputId, float opacity = 1f, int? rotationOverride = null)
    {
        ...
        foreach ((Point offset, TileKind kind) in _data.Displays[rotation].AsSpan())
        {
            ...
            switch (kind)
            {
                ...
                case TileKind.Input:
                    ...
                    g.ShapeBatch.DrawCircle(mapPos, wireRad * 0.5f, color, outline);
                    break;
                case TileKind.Component when !_isSpecialSprite && !(Descriptor is IntrinsicBlueprint.Disp && sim is not null):
        ...
        if(Descriptor is IntrinsicBlueprint.Disp && sim is not null)
        {
            PowerState powerState = sim.PowerStateAt(Inputs[0] + pos);
            Vector2 mapPos = pos.ToVector2() * scale - new Vector2(scale * 0.5f);

            Vector2 tile = new(scale, scale + 0.1f);

            for (int i = 0; i < 8; i++)
            {
                bool power = (powerState.Values & (1 << i)) != 0;

                g.ShapeBatch.FillRectangle(mapPos + new Vector2(scale * i, 0), tile, power ? Color.White : Color.Black, aaSize: 0);
            }
        }
    ...
    public static readonly Blueprint Splitter = new([
        ...
        (new Point(1, 2), TileKind.Output),
        (new Point(1, 3), TileKind.Output),
    ], nameof(Splitter), IntrinsicBlueprint.Splitter);

    public static readonly Blueprint Joiner = new([
        (new Point(0, -4), TileKind.Component),
        (new Point(0, -3), TileKind.Component),
        (new Point(0, -2), TileKind.Component),
        (new Point(0, -1), TileKind.Component),
        ...
        (new Point(-1, 2), TileKind.Input),
        (new Point(-1, 3), TileKind.Input),
    ], nameof(Joiner), IntrinsicBlueprint.Joiner);

    public static readonly Blueprint Disp = new([
        (new Point(0, 0), TileKind.Component),
        (new Point(-1, 0), TileKind.Input),
